...
object WorkFlowUtil {
  ...
  /**
    * The purpose of this method is to compare "5.0" and 5 and ensure they are equal.
    *
    * @param srcValue
    * @param destValue
    * @return
  ...
  private def handleDecimalZero(srcValue: String, destValue: String, destColumnType: String): Boolean = {
    val res = try {

      destColumnType match {
        case "int" => destValue.toInt == srcValue.toInt
        case "float" => destValue.toFloat == srcValue.toFloat
        case "long" => destValue.toLong == srcValue.toLong
        case "double" => destValue.toDouble == srcValue.toDouble
        case "short" => destValue.toShort == srcValue.toShort
        case "decimal" => destValue.toDouble == srcValue.toDouble
        case "boolean" => destValue.toBoolean == srcValue.toBoolean
        case _ => destValue == srcValue
      }
